  ...
  return (
    <>
      <div className="mx-auto grid max-w-5xl gap-6 md:hidden">
        {mobileReviews.map((review) => <Testimonial key={review.id} {...review} avatarClassName="h-14 w-14" className="h-auto p-7" />)}
      </div>
      <div className="mx-auto hidden max-w-5xl grid-cols-1 gap-7 md:grid">
        {desktopReviews.map((review) => <Testimonial key={review.id} {...review} avatarClassName="h-16 w-16" className="h-auto p-10 [&>p]:text-lg" />)}
      </div>
